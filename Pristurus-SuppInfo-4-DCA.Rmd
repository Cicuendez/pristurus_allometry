...
fit.0 <- lm.rrpp(shape~habitat, data = rdf) #H_0: group differences; no allometry 
pw.hab <- pairwise(fit.hab, fit.null = fit.0, groups = rdf$habitat, 
                   covariate = rdf$svl)
  slp.hab <- pw.hab$slopes[[1]] 

#Test 
slopes <- lapply(1:1000, function(j) rbind(coef.evol,pw.hab$slopes[[j]]))
slp.ang <- lapply(1:1000, function(j) acos(RRPP:::vec.cor.matrix(slopes[[j]]))*
                    180/pi)
