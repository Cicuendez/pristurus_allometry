...
Next, because allometry describes the extent to which traits covary with size and with each other (i.e., integration), we conducted an analysis of integration. Here we characterized the extent of morphological integration in body form for individuals within each habitat group. Integration was estimated using the relative eigenvalue variance [$V_{rel}$: @Pavlicev2009], which summarizes the dispersion of eigenvalues of the trait covariance matrix [see @ConawayAdams2022]. To compare the strength of morphological integration across habitat groups, we converted $V_{rel}$ to an effect size ($Z$-score) which measures the strength of integration [@ConawayAdams2022], and performed a series of two-sample tests to compare effect sizes among habitat groups. Additionally and for comparison, we repeated these analyses on the set of size-standardized trait data, found as a set of shape ratios [sensu @Mosimann1970] where each trait was divided by body size (Supplemental Information). \hfill\break 
