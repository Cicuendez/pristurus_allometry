...
%DIF DEL PristurusMS-3-DCA.tex   Thu Oct 27 14:49:43 2022
%DIF ADD PristurusMS-3.tex       Wed Oct 26 16:16:57 2022
...
%DIF -------
  pdftitle={The role of habitat and evolutionary allometry in the morphological differentiation of Pristurus geckos (OR: Evolution along allometric lines of least resistance: Morphological differentiation in Pristurus geckos)}, %DIF > 
%DIF -------
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{\DIFaddbegin \DIFadd{The role of habitat and evolutionary allometry in the
morphological differentiation of \emph{Pristurus} geckos (OR: }\DIFaddend Evolution
along allometric lines of least resistance: Morphological
differentiation in \emph{Pristurus} geckos\DIFaddbegin \DIFadd{)}\DIFaddend }
\author{}
\date{\vspace{-2.5em}}
...
\begin{center}
\textbf{\DIFaddbegin \DIFadd{ORDER TBD:  }\DIFaddend H{\'{e}}ctor Tejero-Cicu{\'{e}}ndez$^{1,*}$,  Iris Men{\'{e}}ndez$^{2,3}$, Adri{\'{a}}n Talavera, Gabriel Riaño, Marc Sim{\'{o}}-Riudalbas$^{1}$, Salvador Carranza$^{1}$, and Dean C. Adams$^{4}$}
\end{center}

\begin{center}\DIFdelbegin \DIFdel{27 }\DIFdelend \DIFaddbegin \DIFadd{26 }\DIFaddend October, 2022\end{center}
...
We then examined changes in the size-corrected body shape proportions
...
proportionality across the phylogeny for the group. \hfill\break

Furthermore, we examined the relationship between evolutionary and
static allometry. Species-specific slopes describing the extent of head
and limb allometry within each species were extracted from an analysis
of covariance modeled as: \(PLS1_{head} \sim SVL*species\) and
\(PLS1_{limb} \sim SVL*species\) respectively. This species-specific
allometric slopes (static allometry) were then compared to the
evolutionary allometric slopes expected according to their habitat by
...
slopes changed throughout the phylogeny of \emph{Pristurus} under a
Brownian motion model (for a similar approach see Adams and Nistri
2010). \hfill\break
...
species are variable in this trait, with \emph{P. insignis} exhibiting
large and \emph{P. insignoides} small limb proportions. For small
...
(\(\rho = 0.42\)). \hfill\break

We found that static allometric slopes (i.e., per-species slopes) are in
...
traits occurred repeatedly. \hfill\break
...
in the geckos of the genus \emph{Pristurus}. In particular, we
...
\emph{Pristurus} present proportionally larger heads and longer limbs
relative to other large species, while small species in the ground have
proportionally smaller heads and shorter limbs (\textbf{Figure residuals
traitgrams}). This is consistent with previous results on the
...
represent an evolutionary mapping of \DIFdelbegin \DIFdel{residuals from phylogenetic
regressions }\DIFdelend \DIFaddbegin \DIFadd{regression slopes }\DIFaddend describing the
...
Figure 4. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Figure 5. }\DIFdelend Phylomorphospace of \emph{Pristurus}, based on residuals from
...
\DIFdelbegin \DIFdel{Figure 6. Representative specimens from large and small
}\textit{\DIFdel{Pristurus}} %DIFAUXCMD
\DIFdel{species, colored by habitat use: ground (beige) and
...
\caption{Linear Measurements used in this study. SVL = snout-vent length, TL = trunk length, HL = head length, HW = head width, HH = head height, Lhu = humerus length, Lun = ulna length, Lfe = femur length, Ltb = tibia length (for details see Tejero-Cicu{\DIFdelbeginFL \DIFdelFL{\'{e}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{'}{\DIFaddFL{e}\DIFaddendFL }\DIFaddbeginFL }\DIFaddendFL ndez et al. 2021a).}\label{fig:unnamed-chunk-3}
\end{figure}
...
%DIFDELCMD < {\centering %%%
\DIFdelendFL \includegraphics[width=1\linewidth]{Figs/figure_2_ggplot} \DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelendFL \caption{Plot of regression scores and predicted lines representing the relationship between linear body measurements and size (SVL). Individuals are colored by habitat use: ground (beige), rock (dark purple), and tree (magenta). Isometric trend represented by the dashed line.}\label{fig:unnamed-chunk-4}
\end{figure}
...
%DIFDELCMD < {\centering %%%
\DIFdelendFL \includegraphics[width=1\linewidth]{Figs/figure_3_phenogram_RESIDUALS} \DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelendFL \caption{Traitgrams showing the evolution of body size (SVL) through time based on the phylogenetic tree of \textit{Pristurus}. Colors represent an evolutionary mapping of \DIFdelbeginFL \DIFdelFL{residuals from phylogenetic regressions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{regression slopes }\DIFaddendFL describing the relationship of (A) head morphology versus body size, and (B) limb proportions versus body size (see text for descriptions). Species names are colored by habitat use: ground (beige), rock (dark purple), and tree (magenta).}\label{fig:unnamed-chunk-5}
\end{figure}
...
%DIFDELCMD < {\centering %%%
\DIFdelendFL \includegraphics[width=1\linewidth]{Figs/figure_4_static_allometry} \DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelendFL \caption{adsf}\label{fig:unnamed-chunk-6}
\end{figure}
...
%DIFDELCMD < {\centering %%%
\DIFdelendFL \includegraphics[width=1\linewidth]{Figs/figure_5_phylomorphospace_large_small} \DIFdelbeginFL %DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelendFL \caption{Phylomorphospace of \textit{Pristurus}, based on residuals from a phylogenetic regression of body measurements on size (SVL). Species means are colored by habitat use: ground (beige), rock (dark purple), and tree (magenta). Large and small rock-dwelling and ground-dwelling are highlighted with darker colors to highlight their differentiation and relative positions in morphospace.}\label{fig:unnamed-chunk-7}
...
%DIFDELCMD < {\centering \includegraphics[width=98.18in,height=0.9\textheight]{Figs/figure_6_Pristurus_allometry_rescaledSVL_1cm} 
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Representative specimens from large and small }\textit{\DIFdelFL{Pristurus}} %DIFAUXCMD
\DIFdelFL{species, colored by habitat use: ground (beige) and rock (dark purple). Specimens are scaled to a common body size (SVL) to emphasize the relative differences in limb and head proportions. Original scale shown as the gray bar.}}%DIFAUXCMD
%DIFDELCMD < \label{fig:unnamed-chunk-8}
%DIFDELCMD < %%%
